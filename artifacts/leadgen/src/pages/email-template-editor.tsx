...
type SendFormat = "plain_text" | "html";
...
const EMPTY_FORM: FormState = {
  name: "",
  subject: "",
  body: "Hi {{company_name}},\n\nI wanted to reach out about your work at {{website_url}}.\n\nBest regards,",
  personalizationPrompt: "",
  attachments: [],
  sendFormat: "plain_text",
  isActive: true,
};
...
function EmailPreview({ subject, body, attachments, sendFormat }: { subject: string; body: string; attachments: TemplateAttachment[]; sendFormat: SendFormat }) {
  const isPlainText = sendFormat === "plain_text";
  return (
    <Card className="glass-card">
      <CardHeader className="pb-3">
        <CardTitle className="text-base font-semibold flex items-center gap-2">
          ...
          Live Preview
          <span className={`ml-auto text-[10px] font-normal px-1.5 py-0.5 rounded-md ${isPlainText ? "bg-green-500/10 text-green-600" : "bg-primary/10 text-primary"}`}>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="rounded-xl border border-border/50 bg-background p-4 min-h-[360px]">
          {body.trim() ? (
            isPlainText ? (
              <pre className="text-sm leading-relaxed font-sans whitespace-pre-wrap break-words m-0">
                {renderSample(body)}
              </pre>
            ) : (
              <div
...
export function EmailTemplateEditor() {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      sendFormat: (template.sendFormat === "html" ? "html" : "plain_text") as SendFormat,
  ...
  const preview = useMemo(
    () => ({ subject: form.subject, body: form.body, attachments: completeAttachments, sendFormat: form.sendFormat }),
    [completeAttachments, form.body, form.subject, form.sendFormat],
  );
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Compose your email and preview the result before saving. Choose Plain Text for cold outreach.</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_420px] gap-6 items-start">
        <Card className="glass-card">
          <CardHeader className="pb-3">
            <CardTitle className="text-base font-semibold flex items-center gap-2">
              ...
              {form.sendFormat === "html" ? "HTML Editor" : "Template Editor"}
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-5">
            ...
            {/* ── Send Format selector ──────────────────────────────── */}
            <div className="space-y-2">
              <Label className="text-xs font-medium">Send Format</Label>
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => set("sendFormat", "plain_text")}
                  className={`flex-1 rounded-xl border px-3 py-2 text-sm font-medium transition-colors ${form.sendFormat === "plain_text" ? "border-primary bg-primary/10 text-primary" : "border-border/60 text-muted-foreground hover:bg-muted/40"}`}
                >
                  Plain Text
                </button>
                <button
                  type="button"
                  onClick={() => set("sendFormat", "html")}
                  className={`flex-1 rounded-xl border px-3 py-2 text-sm font-medium transition-colors ${form.sendFormat === "html" ? "border-primary bg-primary/10 text-primary" : "border-border/60 text-muted-foreground hover:bg-muted/40"}`}
                >
                  HTML
                </button>
              </div>
              <p className="text-[11px] text-muted-foreground">
                {form.sendFormat === "plain_text"
                  ? "Sends as text/plain — no HTML tags, better deliverability for cold outreach."
                  : "Sends as multipart/alternative with HTML — supports bold, links, images."}
              </p>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <Label htmlFor="template-body" className="text-xs font-medium">
                  {form.sendFormat === "html" ? "HTML Body *" : "Email Body *"}
                </Label>
                {form.sendFormat === "html" && (
              ...
              <Textarea
                ...
                placeholder={form.sendFormat === "plain_text" ? "Hi {{company_name}},\n\nYour message here.\n\nBest regards," : "<p>Hi {{company_name}},</p>"}
        ...
        <div className="xl:sticky xl:top-6">
          <EmailPreview subject={preview.subject} body={preview.body} attachments={preview.attachments} sendFormat={preview.sendFormat} />
        </div>
      </div>
    </div>
  );
